...
const OudsPage = () => {
    ...
    const oudsProducts = products.filter(
        (product: ProductCardProps) => product.category?.toLowerCase().includes("oud") ||
            product.category?.toLowerCase().includes("ouds")
    );

    if (error || oudsProducts.length === 0) {
        return (
    ...
    return (
        <div className="w-full bg-gray-100 pt-14 flex flex-col gap-4 pb-6">
            <StickyPageHeader title="Ouds Products" path="Home > Ouds" />

            <div className="w-full  flex flex-wrap justify-center items-center gap-4">
                {products
                    .filter((product: ProductCardProps) => product.category?.toLowerCase().includes("ouds") ||
                        product.category?.toLowerCase().includes("oud"))
