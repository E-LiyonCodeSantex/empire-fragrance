...
const LuxuryPerfumePage = () => {
    ...
    const luxuryProducts = products.filter(
        (product: ProductCardProps) => product.category?.toLowerCase().includes("luxury")
    );
    ...
    return (
        <div className="w-full bg-gray-100 pt-14 flex flex-col gap-4 pb-6">
            <StickyPageHeader title="Luxury Products" path="Home > Luxury Perfumes" />

            <div className="w-full  flex flex-wrap justify-center items-center gap-4">
                {products
                    .filter((product: ProductCardProps) => product.category?.toLowerCase().includes("luxury"))
