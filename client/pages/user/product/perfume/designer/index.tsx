...
const DesignerPerfumePage = () => {
    ...
    const designerProducts = products.filter(
        (product: ProductCardProps) => product.category?.toLowerCase().includes("designer")
    );

    if (error || designerProducts.length === 0) {
        return (
    ...
    return (
        <div className="w-full bg-gray-100 pt-14 flex flex-col gap-4 pb-6">
            <StickyPageHeader title="Designers Products" path="Home > Designers Perfumes" />

            <div className="w-full  flex flex-wrap justify-center items-center gap-4">
                {products
                    .filter((product: ProductCardProps) => product.category?.toLowerCase().includes("designer"))
