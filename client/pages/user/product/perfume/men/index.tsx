...
const MenPerfumePage = () => {
    ...
    if (isLoading) {
        return (
            <div className="w-full h-screen flex justify-start items-center text-gray-600">
  ...
  /(\bmen\b)/i.test(product.category || "")
);
    ...
    return (
        <div className="w-full bg-gray-100 pt-14 flex flex-col gap-4 pb-6">
            <StickyPageHeader title="Men Products" path="Home > Men Perfumes" />

            <div className="w-full  flex flex-wrap justify-center items-center gap-4">
                {products
                    .filter((product: ProductCardProps) => product.category?.toLowerCase() === "men" ||
                     /(\bmen\b)/i.test(product.category || ""))
