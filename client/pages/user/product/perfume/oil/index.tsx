...
const OilPerfumePage = () => {
    ...
    const oilProducts = products.filter(
        (product: ProductCardProps) => product.category?.toLowerCase().includes("oil")
    );

    if (error || oilProducts.length === 0) {
        return (
    ...
    return (
        <div className="w-full bg-gray-100 pt-14 flex flex-col gap-4 pb-6">
            <StickyPageHeader title="Oil Perfume Products" path="Home > Oil Perfumes" />

            <div className="w-full  flex flex-wrap justify-center items-center gap-4">
                {products
                    .filter((product: ProductCardProps) => product.category?.toLowerCase().includes("oil"))
