...
const UnisexPerfumePage = () => {
    ...
    const unisexProducts = products.filter(
        (product: ProductCardProps) => product.category?.toLowerCase().includes("unisex")
    );

    if (error || unisexProducts.length === 0) {
        return (
    ...
    return (
        <div className="w-full bg-gray-100 pt-14 flex flex-col gap-4 pb-6">
            <StickyPageHeader title="Designers Products" path="Home > Unisex" />

            <div className="w-full  flex flex-wrap justify-center items-center gap-4">
                {products
                    .filter((product: ProductCardProps) => product.category?.toLowerCase().includes("unisex"))
